export const en = {
  // Navigation
  nav: {
    home: "Home",
    jobs: "Jobs",
    courses: "Courses",
    atoms: "Atoms",
    profile: "Profile",
    settings: "Settings",
    help: "Help Center",
    notifications: "Notifications",
    yourJobs: "Your jobs",
    members: "Members",
    interviews: "Interviews",
  },
  // Common
  common: {
    loading: "Loading...",
    error: "Error",
    success: "Success",
    save: "Save",
    cancel: "Cancel",
    delete: "Delete",
    edit: "Edit",
    back: "Back",
    next: "Next",
    previous: "Previous",
    submit: "Submit",
    search: "Search",
    filter: "Filter",
    clear: "Clear",
    apply: "Apply",
    close: "Close",
    yes: "Yes",
    no: "No",
    ok: "OK",
    view: "View",
    more: "More",
    less: "Less",
  },
  // Auth
  auth: {
    login: "Login",
    signup: "Sign Up",
    logout: "Logout",
    email: "Email",
    password: "Password",
    confirmPassword: "Confirm Password",
    forgotPassword: "Forgot Password?",
    resetPassword: "Reset Password",
    rememberMe: "Remember me",
    createAccount: "Create Account",
    alreadyHaveAccount: "Already have an account?",
    dontHaveAccount: "Don't have an account?",
    firstName: "First Name",
    lastName: "Last Name",
    phone: "Phone",
    welcome: "Welcome",
    welcomeBack: "Welcome Back",
  },
  // Dashboard
  dashboard: {
    welcome: "Welcome",
    jobOffers: "Job Offers",
    courses: "Courses",
    skillTest: "Skill Test",
    skillPath: "Skill Path",
    interests: "Interests",
  },
  // Jobs
  jobs: {
    title: "Jobs",
    noJobs: "No jobs found",
    apply: "Apply",
    applied: "Applied",
    save: "Save",
    saved: "Saved",
    location: "Location",
    salary: "Salary",
    salaryRange: "Salary Range",
    minimum: "Minimum",
    maximum: "Maximum",
    byAgreement: "By Agreement",
    workType: "Work Type",
    country: "Country",
    city: "City",
    searchCities: "Search cities...",
    selectAllCities: "Select all",
    noCitiesFound: "No cities found",
    citiesSelected: "cities selected",
    choose: "Choose",
    saveThisFilter: "Save this filter",
    activeFilters: "Active Filters",
    clearAllFilters: "Clear all filters",
    interests: "Interests / Skills",
    type: "Type",
    posted: "Posted",
    deadline: "Deadline",
    description: "Description",
    requirements: "Requirements",
    benefits: "Benefits",
    company: "Company",
    fullTime: "Full Time",
    partTime: "Part Time",
    contract: "Contract",
    remote: "Remote",
    hybrid: "Hybrid",
    filter: "Filter Jobs",
    clearFilters: "Clear Filters",
    search: "Search",
    searchPlaceholder: "Search",
    filters: "Filters",
    vacancy: "vacancy",
    vacancies: "Jobs",
    dateDescending: "Date Descending",
    dateAscending: "Date Ascending",
    dateDescendingFull: "Date Descending",
    dateAscendingFull: "Date Ascending",
    tryAdjustingFilters: "Try adjusting your search terms or filters",
    showing: "Showing",
    of: "of",
    page: "Page",
    previous: "Previous",
    next: "Next",
    errorLoading: "Error loading jobs. Please try again.",
  },
  // Courses
  courses: {
    title: "Courses",
    noCourses: "No courses found",
    enroll: "Enroll",
    enrolled: "Enrolled",
    enrollSuccess: "You are enrolled in this course.",
    enrollLoginRequired: "Please log in to enroll.",
    enrollFailed: "Could not enroll. Please try again.",
    enrolledStudentsBox: "Enrolled students",
    enrolledStudentsListTitle: "Students enrolled in this course",
    viewEnrolledStudents: "View students",
    enrolledStudentsModalEmpty: "No students have enrolled in this course yet.",
    save: "Save",
    saved: "Saved",
    duration: "Duration",
    level: "Level",
    instructor: "Instructor",
    academy: "Academy",
    description: "Description",
    curriculum: "Curriculum",
    reviews: "Reviews",
    price: "Price",
    free: "Free",
    filter: "Filter Courses",
    clearFilters: "Clear Filters",
  },
  // Profile
  profile: {
    title: "Profile",
    editProfile: "Edit Profile",
    personalInfo: "Personal Information",
    contactInfo: "Contact Information",
    skills: "Skills",
    experience: "Experience",
    education: "Education",
    bio: "Bio",
    saveChanges: "Save Changes",
  },
  // Settings
  settings: {
    title: "Settings",
    account: "Account",
    preferences: "Preferences",
    notifications: "Notifications",
    privacy: "Privacy",
    language: "Language",
    theme: "Theme",
    darkMode: "Dark Mode",
    lightMode: "Light Mode",
    logout: "Log out",
    sections: {
      account: "Account",
      notifications: "Notifications",
      privacy: "Privacy & Security",
      subscription: "Subscription & Billing",
      accessibility: "Theme & Accessibility",
    },
    list: {
      manageAccount: "Manage your account",
      on: "On",
      off: "Off",
      privacySubtitle: "Password, visibility & data",
      freePlan: "Free Plan",
      logoutSubtitle: "Sign out of your account",
      loading: "Loading...",
    },
    themeMode: {
      light: "Light",
      dark: "Dark",
      auto: "Auto",
      on: "ON",
      off: "OFF",
    },
    fontSize: {
      title: "Font Size",
      small: "Small",
      medium: "Medium",
      large: "Large",
    },
    languageLabel: {
      english: "English",
      georgian: "Georgian",
    },
    notificationsPage: {
      email: "Email",
      inApp: "In-app",
      push: "Push",
      jobMatches: "Job Matches",
      jobMatchesDesc: "Get notified when new jobs match your profile",
      newCourses: "New Courses",
      newCoursesDesc: "Receive updates about new courses in your interests",
      skillUpdates: "Skill Updates",
      skillUpdatesDesc: "Get notified about skill recommendations and updates",
      newsletter: "Newsletter",
      newsletterDesc: "Subscribe to our weekly newsletter",
      messages: "Messages",
      messagesDesc: "Notify me about new messages",
      progressReminders: "Progress Reminders",
      progressRemindersDesc: "Get reminders about your learning progress",
      pushEnabled: "Enable Push Notifications",
      pushEnabledDesc: "Allow Breneo to send you push notifications",
    },
    privacyPage: {
      security: "Security",
      changePassword: "Change password",
      changePasswordDesc: "Update your password with email verification",
      dataVisibility: "Data visibility",
      showSkills: "Show Skills",
      showSkillsDesc: "Display your skills on your profile",
      showTestResults: "Show Test Results",
      showTestResultsDesc: "Display your skill test results",
      showCompletedCourses: "Show Completed Courses",
      showCompletedCoursesDesc: "Display courses you've completed",
    },
    subscriptionPage: {
      currentPlan: "Current plan",
      activeNow: "Active Now",
      nextRenewal: "Next automatic renewal: {date}",
      monthly: "Monthly",
      basicAccess: "Basic access with limited features",
      changePlan: "Change Plan",
      unlockPro: "Unlock Pro Features",
      paymentMethod: "Payment method",
      loadingPaymentMethods: "Loading payment methods...",
      savedViaBog: "Saved via Bank of Georgia",
      default: "Default",
      noPaymentMethods:
        "No payment methods saved. Subscribe to a plan to save your card.",
      billingHistory: "Billing history",
      fetchingHistory: "Fetching transaction history...",
      subscriptionPayment: "Subscription Payment",
      noHistoryTitle: "No transaction history found",
      noHistoryDesc:
        "When you start subscribing to plans, your payment history will appear here.",
      card: "Card",
    },
    accountPage: {
      resumeDescription:
        "Upload a PDF resume to auto-fill your summary, education, work experience, and skills. Existing entries won't be duplicated.",
      uploadResume: "Upload Resume",
      uploadingResume: "Uploading Resume...",
      premiumFeature: "Premium feature",
      academyInfo: "Academy information",
      academyName: "Academy name",
      academyNamePlaceholder: "Enter your academy name",
      phoneNumber: "Phone number",
      phonePlaceholder: "Enter your phone number",
      phoneInvalid: "Enter a valid phone number",
      contactEmail: "Contact email",
      contactEmailPlaceholder: "contact@academy.com",
      website: "Website",
      websitePlaceholder: "https://www.academy.com",
      academyNameRequired: "Academy name is required.",
      noChanges: "No changes to save.",
      userMissing: "User ID not found. Please log in again.",
      academyLoadError: "Failed to load academy profile.",
      academySaveSuccess: "Academy information updated.",
      academySaveError: "Failed to update academy profile. Please try again.",
      saving: "Saving...",
    },
    changePassword: {
      title: "Change password",
      description: "We'll send a verification code to your email address.",
      email: "Email",
      verificationCode: "Verification code",
      verificationCodePlaceholder: "Enter 6-digit code",
      newPassword: "New password",
      confirmPassword: "Confirm password",
      sendCode: "Send Verification Code",
      sending: "Sending...",
      verifyCode: "Verify Code",
      verifying: "Verifying...",
      updatePassword: "Update Password",
      updating: "Updating...",
    },
    logoutConfirm: {
      title: "Log out",
      description: "Are you sure you want to log out?",
      confirm: "Log out",
    },
    sectionNotFound: "Section not found",
    downloadApp: {
      badge: "Mobile app",
      title: "Download Breneo app",
      description:
        "Install Breneo for quick access to jobs, courses, and notifications from your home screen.",
      faster: "Faster, app-like experience",
      offline: "Works offline when you need it",
      jobMatches: "Stay on top of job matches",
      preparing: "Preparing...",
      installNow: "Install now",
      downloadApp: "Download app",
      installStarted: "App installation started!",
      installCancelled: "Installation cancelled. You can try again later.",
      openingBrowser:
        "Opening your browser to continue installation...",
      installUnavailable:
        "Install is not available right now. Open this page in Safari or Chrome and try again.",
      iosGuide: {
        title: "How to install on iPhone",
        step1Before: "Tap the ",
        share: "Share",
        step1After: " button (below)",
        step2Before: "Select ",
        addToHomeScreen: "Add to Home Screen",
        step3Before: "Tap ",
        add: "Add",
        step3After: " — you're ready!",
        gotIt: "Got it",
      },
    },
    updateApp: {
      title: "Update available",
      description:
        "Version {version} is available. Update now to get the latest features and fixes.",
      updateApp: "Update app",
      updating: "Updating...",
    },
    appVersion: {
      version: "Version {version}",
      updateApp: "Update app",
    },
  },
  // Notifications
  notifications: {
    title: "Notifications",
    noNotifications: "No notifications",
    markAsRead: "Mark as read",
    markAllAsRead: "Mark all as read",
    removeSelected: "Remove selected",
    selectedCount: "{count} selected",
    tabNotifications: "Notifications",
    tabCvViews: "CV views",
    tabJoinRequests: "Join requests",
    noCvViews: "No CV views yet",
    noJoinRequests: "No pending join requests",
    acceptJoinRequest: "Accept",
    joinRequestPending: "Pending",
    joinRequestWantsToJoin: "Wants to join {company}",
    joinRequestApproved: "{name} was added to your company.",
    browserNotifications: "Browser Notifications",
    enableBrowserNotifications: "Enable Browser Notifications",
    browserNotificationsDescription:
      "Get browser alerts when you receive new notifications, including job matches and account updates.",
    notificationPermissionGranted: "Browser notifications are enabled",
    notificationPermissionDenied:
      "Browser notifications are blocked. Please enable them in your browser settings.",
    notificationPermissionDefault:
      "Click the button below to enable browser notifications.",
    requestPermission: "Enable Notifications",
    checkingForJobs: "Checking for new jobs...",
    notificationsEnabled: "Notifications Enabled",
    notificationsDisabled: "Notifications Disabled",
  },
  // Subscription
  subscription: {
    title: "Subscription",
    upgrade: "Upgrade plan",
    currentPlan: "Current Plan",
    features: "Features",
    subscribe: "Subscribe",
  },
  // Help
  help: {
    title: "Help Center",
    faq: "FAQ",
    contact: "Contact Support",
  },
  // Terms
  terms: {
    title: "Terms of Use",
    lastUpdated: "Last updated",
  },
  // Skill Test
  skillTest: {
    title: "Skill Test",
    start: "Start Test",
    next: "Next",
    previous: "Previous",
    finish: "Finish",
    results: "Results",
  },
  // Skill Path
  skillPath: {
    title: "Your Personalized Skill Path",
  },
  // Home CTAs
  home: {
    skillTestTitle: "Begin Your Skill Assessment Journey",
    skillTestSubtitle:
      "Take our skill test to unlock personalized recommendations.",
    skillPathTitle: "Explore your personalized career path",
    skillPathSubtitle: "Get personalized recommendations for your career.",
  },
  // Errors
  errors: {
    somethingWentWrong: "Something went wrong",
    tryAgain: "Try again",
    notFound: "Not found",
    pageNotFound: "Page Not Found",
    pageNotFoundDescription:
      "The page you're looking for doesn't exist or has been moved.",
  },
  // Job match (detail page)
  jobMatch: {
    expLevel: "Exp. Level",
    skill: "Skill",
    industryExp: "Industry Exp.",
    overall: "Overall",
    excellentMatch: "Excellent match",
    bestMatch: "Best match",
    goodMatch: "Good match",
    fairMatch: "Fair match",
    badMatch: "Bad match",
    scoreDescription:
      "This score shows how closely your skills match the job requirements, prioritizing exact and key skill matches.",
  },
  mockInterview: {
    badge: "AI practice",
    title: "AI-based mock interview",
    description:
      "Practice for this role with a virtual HR interviewer — 3 tailored questions, live camera, and instant feedback on every answer.",
    roleHint: "Interview practice for: {role}",
    startButton: "Start interview now",
    headerButton: "AI Interview",
  },
  // Webinars
  webinars: {
    title: "Webinars",
    noWebinars: "No webinars found",
    upcoming: "Upcoming",
    past: "Past",
    register: "Register",
    registered: "Registered",
    comingsoon: "Coming Soon",
    comingsoondescription:
      "We’re adding a brand new page to Breneo very soon! Stay tuned for fresh features and updates that will make your experience even better.",
  },
  atoms: {
    title: "Micro-learning Atoms",
    headerTitle: "Learning Atoms",
    choosePath: "Choose a career path to start your next atom lesson.",
    pathSubtitle:
      "Follow your atom path lesson by lesson. Complete each quiz to unlock the next atom.",
    readyToLearn: "Up next",
    startLesson: "Start lesson",
    noAtomsTitle: "No lessons yet",
    noAtoms: "This career path does not have any atoms yet.",
    professionNotFound: "Career path not found.",
    loadError: "Could not load the next lesson. Please try again.",
    errorTitle: "Something went wrong",
    chooseAnotherPath: "Choose another path",
    tryAgain: "Try again",
    passedToast: "Great job! Atom completed.",
    failedToast: "Not quite — review the explanation and try again.",
    quizAnnouncement: "Time to check your knowledge",
    startQuiz: "Start quiz",
    quizDefaultQuestion: "Check your understanding",
    nextCard: "Next",
    previousCard: "Back",
    swipeHint: "Swipe left for next · Swipe right to go back",
    tapHint: "Tap the right side for next · Tap the left side to go back",
    tapToBegin: "Tap anywhere to begin this lesson",
    tapForNextCard: "Tap anywhere to view the next card",
    statusCompleted: "Completed",
    statusAvailable: "Start",
    statusLocked: "Locked",
    currentAtomHint: "Tap the glowing atom below to continue your path.",
  },
  employerJobForm: {
    extractingDescriptionTitle: "Extracting job description…",
    extractingDescriptionHint:
      "We’re analyzing your text with AI. This usually takes a few seconds. English and Georgian descriptions are both supported.",
    buttonExtractingSaving: "Extracting & saving…",
    buttonSaving: "Saving…",
    fullDescriptionLabel: "Full job description",
    responsibilitiesLabel: "Responsibilities",
    responsibilitiesHint:
      "What the hire will do — one bullet per line (max 6). Pre-filled from AI; you can edit.",
    qualificationsLabel: "Qualifications",
    qualificationsHint:
      "Requirements and experience — one bullet per line (max 6). Pre-filled from AI; you can edit.",
    skillsLabel: "Required skills",
    skillsEditHint:
      "Skills used for candidate matching. Search and add the abilities this role requires.",
    skillsManualHint:
      "AI could not detect skills. Add the skills required for candidate matching.",
  },
  // Beta Version
  beta: {
    title: "You’re using the beta version",
    message:
      "Some features may be limited or change as we continue improving the platform. We appreciate your feedback. it helps us build a better experience.",
  },
};

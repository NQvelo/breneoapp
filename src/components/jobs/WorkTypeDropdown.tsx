...
export const WorkTypeDropdown: React.FC<WorkTypeDropdownProps> = ({
  ...
  const [tempIsRemote, setTempIsRemote] = useState<boolean>(isRemote);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTempIsRemote(isRemote);
    }
  }, [isOpen, selectedWorkTypes, isRemote]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        setIsOpen(false);
        setTempSelectedWorkTypes(selectedWorkTypes); // Reset temp state on cancel
        setTempIsRemote(isRemote);
      }
    };
  ...
  const handleSelectAll = () => {
    if (tempSelectedWorkTypes.length === workTypes.length) {
      setTempSelectedWorkTypes([]);
    } else {
      setTempSelectedWorkTypes(workTypes.map((type) => type.id));
    }
  };

  const handleSave = () => {
    onWorkTypesChange(tempSelectedWorkTypes);
    if (onRemoteChange) {
      onRemoteChange(tempIsRemote);
    }
    setIsOpen(false);
  };

  const isAllSelected = tempSelectedWorkTypes.length === workTypes.length;
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            ...
            {workTypes.map((workType) => {
              const isChecked = tempSelectedWorkTypes.includes(workType.id);
              return (
                <div
                  key={workType.id}
                  className={cn(
            ...
            {onRemoteChange && (
              <>
                ...
                <div
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setTempIsRemote(!tempIsRemote);
                  }}
                >
                  <Checkbox
                    checked={tempIsRemote}
                    onCheckedChange={(checked) => {
                      setTempIsRemote(!!checked);
                    }}
                    className="mr-3"
                  />
                  <span
